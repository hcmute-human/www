...
interface Props {
  row: TRow<DepartmentPosition>;
}

export default function PositionRow({ row }: Props) {
  ...
  return (
    <Row className={clsx(isDeleting && 'hidden')}>
      ...
      <Cell key="createdTime">{formatRelativeTimeFromNow(new Date(row.getValue<string>('createdTime')))}</Cell>
      <Cell key="actions">
        <div className="flex items-center gap-2 w-max">
          <Button
            as="link"
            href={`/positions/${row.getValue<string>('id')}`}
            size="sm"
