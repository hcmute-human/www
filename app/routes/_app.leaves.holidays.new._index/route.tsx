...
export async function loader({ request, context: { session } }: LoaderFunctionArgs) {
  ...
  const title = await i18next.getFixedT(request, 'leaves.holidays.new').then((t) => t('meta.title'));
  return json({ title });
}
...
export default function Route() {
  const { t } = useTranslation('leaves.holidays.new');
...
export async function action({ request, context: { session } }: ActionFunctionArgs) {
  const t = await i18next.getFixedT(request, 'leaves.holidays.new');
  const api = SessionApiClient.from(session);
  ...
  if (!api.authorize({ permissions: ['create:holiday'] })) {
    return json(fail({ form: [t('forbidden')] }, submission));
  }

  const result = await SessionApiClient.from(session).post('holidays', {
