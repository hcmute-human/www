...
export default function EmployeeTable() {
  ...
  const { totalCount, items } = useAsyncValue() as Paginated<LeaveType>;
    ...
    <>
        ...
        >
          ...
          <TableBody>
            {table.getRowModel().rows.length === 0 ? (
              ...
              table.getRowModel().rows.map((row) => <LeaveTypeRow row={row} key={row.getValue<string>('id')} />)
            )}
          </TableBody>
        </Table>
      </div>
      ...
      >
        <div className="flex gap-2 items-center min-w-max">
          <Trans
            i18nKey="displaySize"
            defaults="Displaying <select></select> out of {{totalCount}} leave types."
            t={t}
            values={{ totalCount }}
            components={{
              select: (
