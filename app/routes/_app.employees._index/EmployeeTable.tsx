...
export default function EmployeeTable() {
  ...
  const { totalCount, items } = useAsyncValue() as GetEmployeesResult;
  const [searchParams] = useSearchParams();
  const [{ name, size }] = useSearchParamsOr({ name: '', size: 10 });
  const { state } = useNavigation();
  const submitting = state === 'submitting';
  const sortedEmployees = items;
    ...
    <>
        ...
        >
          ...
          <TableBody>
            {table.getRowModel().rows.length === 0 ? (
              <Row>
                <Cell colSpan={100}>No employees found.</Cell>
              </Row>
            ) : (
              table.getRowModel().rows.map((row) => <EmployeeRow row={row} key={row.getValue<string>('id')} />)
            )}
          </TableBody>
        </Table>
      </div>
      ...
      >
        <div className="flex gap-2 items-center min-w-max">
          <Trans
            i18nKey="displaySize"
            defaults="Displaying <select></select> out of {{totalCount}} employees."
            t={t}
            values={{ totalCount }}
            components={{
              select: (
